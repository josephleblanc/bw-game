...
pub fn run(records_path: &Path, since: &str) -> Result<()> {
    ...
    let budgets_path = crate::perf::workspace_root()
        .join("perf")
        .join("budgets.toml");
    let loaded = budgets::load(&budgets_path).ok();
    let markdown = render(&window, loaded.as_ref(), since);
    println!("{markdown}");
    Ok(())
}
...
/// Render the report: budget state (latest record vs budgets.toml), the
/// window's first→last deltas within one environment, flagged findings
/// each paired with a recommended action (D9 invariant), and the latest
/// record's skip log.
fn render(window: &[Record], budgets: Option<&Budgets>, since: &str) -> String {
...
mod tests {
    ...
    fn render_flags_double_digit_growth_with_action() {
        ...
        let markdown = render(&[base, last], None, "7d");
    ...
    fn render_flags_env_mismatch_instead_of_diffing() {
        ...
        let markdown = render(&[other_env, last], None, "30d");
    ...
    fn render_lists_skips_from_latest_record() {
        let markdown = render(&[fixture_record(1_000, 1, 1)], None, "7d");
